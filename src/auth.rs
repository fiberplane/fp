...
pub async fn handle_login_command(args: Arguments) -> Result<()> {
    ...
    let mut config = Config::load(args.profile.clone()).await?;
    ...
    match rx.recv().await {
        Ok(token) => {
            ...
            match config.save(args.profile.as_deref()).await {
                ...
                Err(e) => error!("Error saving API token to profile file: {e:?}"),
            };
        }
...
pub async fn handle_logout_command(args: Arguments) -> Result<()> {
    let mut config = Config::load(args.profile.clone()).await?;

    if let Some(token) = args.token {
        let client = api_client_configuration_from_token(&token, config.base_url(args.base_url)?)?;
