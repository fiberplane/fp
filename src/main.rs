...
async fn main() {
    ...
    // TODO: Add timeout
    match version_check_result.await {
        Ok(Some(new_version)) => {
            eprint!("A new version of fp is available (version: {}). Use `fp update` to update your current fp binary", new_version);
        }
        Ok(None) => trace!("version check skipped or no new version available"),
        Err(err) => warn!(%err, "version check failed"),
    }
}
...
pub async fn background_version_check() -> Result<Option<String>> {
    let check_file = ProjectDirs::from("com", "Fiberplane", "fiberplane-cli")
        .unwrap()
        .config_dir()
        .join("version_check");
    ...
    if !should_check {
        trace!("skipping checking the remote version");
        return Ok(None);
    }
    ...
    // TODO: Do we need to create config_dir if it doesn't exist?
    ...
    OpenOptions::new()
        ...
        .truncate(true)
        .open(&check_file)
        .with_context(|| format!("unable to create/truncate file: {:?}", check_file))?;
